...
static constexpr NWidgetPart _nested_load_font_dialog_widgets[] = {
	...
	NWidget(NWID_HORIZONTAL, NC_EQUALSIZE),
		...
		NWidget(NWID_VERTICAL),
			NWidget(WWT_PANEL, COLOUR_GREY, WID_SL_FONT_PREVIEW_PANEL), SetResize(1, 1), SetFill(1, 1),
			EndContainer(),
...
public:
			...
			: Window(desc), filename_editbox(64), abstract_filetype(abstract_filetype), fop(fop), filter_editbox(EDITBOX_MAX_SIZE)
	{
		...
		switch (this->abstract_filetype) {
			...
			case FT_FONT:
				caption_string = STR_SAVELOAD_LOAD_FONT_CAPTION;
				break;
	...
	void DrawWidget(const Rect &r, WidgetID widget) const override
	{
		switch (widget) {
			...
			case WID_SL_FONT_PREVIEW_PANEL:
				this->DrawFontPreview(r);
				break;
		}
	}
	...
	void DrawFontPreview(const Rect &rect) const
	{
		StringID font_description = STR_EMPTY;

		switch(this->fop) {
			case SLO_LOAD_SMALL_FONT:
				font_description = STR_GAME_OPTIONS_LOAD_SMALL_FONT_TOOLTIP;
				break;
			case SLO_LOAD_MEDIUM_FONT:
				font_description = STR_GAME_OPTIONS_LOAD_MEDIUM_FONT_TOOLTIP;
				break;
			case SLO_LOAD_LARGE_FONT:
				font_description = STR_GAME_OPTIONS_LOAD_LARGE_FONT_TOOLTIP;
				break;
			case SLO_LOAD_MONOSPACED_FONT:
				font_description = STR_GAME_OPTIONS_LOAD_MONOSPACED_FONT_TOOLTIP;
				break;
			default:
				NOT_REACHED();
		}

		Rect paddedDrawArea = rect.Shrink(6);

		DrawStringMultiLine(paddedDrawArea, font_description, TC_BLACK, SA_TOP, false, FS_NORMAL);
	}
