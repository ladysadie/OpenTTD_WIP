...
protected:
	...
	void InitializeCompanyGraphWindow(WindowNumber number)
	{
		...
		this->selector.PreInit(SelectorWidget::company_selector_profile);
...
public:
	...
	{
		selector.UpdateWidgetSize(widget, size, padding, fill, resize);
	...
	{
		selector.OnClick(pt, widget, click_count);
		this->click = pt;
		InvalidateWindowData(this->window_class, this->window_number);
	}
...
struct PaymentRatesGraphWindow : BaseGraphWindow {
			...
			BaseGraphWindow(desc, STR_JUST_CURRENCY_SHORT)
	{
		...
		this->selector.PreInit(SelectorWidget::cargo_selector_profile);
		this->CreateNestedTree();
		this->selector.Init(this);
